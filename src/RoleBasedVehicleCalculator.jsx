      ...
      try {
        ...
        const transformedVehicles = assets.map(asset => {
          ...
          return {
            ...
            ownership: ownership,
            salesPrice: asset.sales_price || 0,
      ...
      {showAddForm && (
        <form onSubmit={handleAddVehicle} className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-2">
            ...
            <div>
              <label className="block text-gray-700 text-sm mb-1">Model</label>
              <input
                type="text"
                value={newModel}
            ...
            <div>
              <label className="block text-gray-700 text-sm mb-1">Sales Price</label>
              <input
                type="number"
                value={newSalesPrice}
          ...
          <div className="mb-4">
            <label className="block text-gray-700 text-sm mb-1">Ownership Type</label>
            <select 
              className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" 
              value={newOwnershipType}
              onChange={(e) => handleAddOwnershipTypeChange(e.target.value)}
            >
          ...
          <div className="mb-4">
            <label className="block text-gray-700 text-sm mb-1">Ownership Shares</label>
            {newShares.map((share, idx) => (
              <div key={idx} className="flex items-center space-x-2 mb-2">
                <input
                  type="text"
                  placeholder="Owner Name"
                  value={share.ownerName}
                  onChange={(e) => handleChangeShare(idx, 'ownerName', e.target.value)}
                ...
                <input
                  ...
                  onChange={(e) => handleChangeShare(idx, 'ownershipPercentage', e.target.value)}
            ...
            {newShares.reduce((sum, s) => sum + Number(s.ownershipPercentage), 0) !== 100 && (
              <div className="text-xs text-red-600 mt-1">Total ownership must equal 100%</div>
            )}
            {newShares.length < 5 && (
              <button 
                type="button"
                onClick={handleAddOwnerToShares}
          ...
          <div className="flex justify-end">
            <button
              ...
              disabled={!newVIN || newVIN.length !== 17 || !newMake || !newModel || !newSalesPrice || 
                        newShares.reduce((sum, s) => sum + Number(s.ownershipPercentage), 0) !== 100}
            >
              Add Vehicle
            </button>
      ...
      {selectedVehicle && (
        <>
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-6">
            <h3 className="font-medium text-blue-700 mb-2">Vehicle Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p className="text-gray-700"><span className="font-medium">VIN:</span> {selectedVehicle.vin}</p>
                <p className="text-gray-700"><span className="font-medium">Vehicle:</span> {selectedVehicle.name}</p>
                {/* Price display with edit functionality for Admin */}
                <div className="mt-2">
                  <p className="text-gray-700 font-medium">Sales Price:</p>
                  {isEditingPrice && userRole === 'Admin' ? (
                    <div className="flex items-center">
                      <input
                        type="number"
                        value={salesPrice}
                        onChange={(e) => setSalesPrice(Number(e.target.value))}
                        className="p-1 border border-gray-300 rounded w-32 mr-2"
                      />
                      <button
                        onClick={finalizePrice}
                  ...
                  {priceStatus === 'Proposed' && (
                    <p className="text-xs text-blue-600 mt-1">Proposed price - needs admin approval</p>
                  )}
                  {priceStatus === 'Finalized' && (
                    <p className="text-xs text-green-600 mt-1">Price updated</p>
                  )}
                </div>
              </div>
              <div>
                <div className="mb-4">
                  <p className="text-gray-700 font-medium mb-1">Ownership Type:</p>
                  {userRole === 'Admin' ? (
                    <select
                      value={ownershipType}
                ...
                <div className="mb-4">
                  <p className="text-gray-700 font-medium mb-1">Ownership Shares:</p>
                  <ul className="list-disc list-inside pl-2">
                    {selectedVehicle.ownershipShares.map((share, idx) => (
                      <li key={idx} className="text-gray-600">
                        {share.ownerName}: {share.ownershipPercentage}%
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                {/* Sale Type Selector */}
                <div className="mb-4">
                  <p className="text-gray-700 font-medium mb-1">Sale Type:</p>
                  <select
                    value={saleType}
            ...
            {userRole === 'Admin' && (
              <div className="mt-4 pt-3 border-t border-gray-200">
                <h3 className="font-medium text-blue-700 mb-2">Admin Controls</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="flex items-center space-x-2 mb-2">
                      <input
                        type="checkbox"
                        checked={waivePackFee}
                        onChange={(e) => setWaivePackFee(e.target.checked)}
                        className="rounded text-blue-600 focus:ring-blue-500"
                      />
                      <span className="text-gray-700">Waive Pack Fee</span>
                    </label>
                    {!waivePackFee && (
                      <div className="flex items-center space-x-2 mb-2">
                        <span className="text-gray-700 w-24">Pack Fee:</span>
                        <input
                          type="number"
                          value={adminPackFee}
                          onChange={(e) => setAdminPackFee(Number(e.target.value))}
                  ...
                  <div>
                    <label className="flex items-center space-x-2 mb-2">
                      <input
                        type="checkbox"
                        checked={waiveBrokerageFee}
                        onChange={(e) => setWaiveBrokerageFee(e.target.checked)}
                        className="rounded text-blue-600 focus:ring-blue-500"
                      />
                      <span className="text-gray-700">Waive Brokerage Fee</span>
                    </label>
                    {!waiveBrokerageFee && (
                      <div className="flex items-center space-x-2 mb-2">
                        <span className="text-gray-700 w-24">Brokerage Fee:</span>
                        <input
                          type="number"
                          value={adminBrokerageFee === null ? '' : adminBrokerageFee}
                          onChange={(e) => setAdminBrokerageFee(e.target.value === '' ? null : Number(e.target.value))}
          ...
          {calculation && (
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-6">
              <h3 className="font-medium text-blue-700 mb-3">Sale Calculation</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between py-1">
                      ...
                      <span className="font-medium">{formatCurrency(salesPrice)}</span>
                    </div>
                    <div className="flex justify-between py-1">
                      <span>Doc Fee</span>
                      <span>{formatCurrency(calculation.doc_fee)}</span>
                    </div>
                    <div className="flex justify-between py-1">
                      <span>Registration Fee</span>
                      <span>{formatCurrency(calculation.registration_fee)}</span>
                    </div>
                    <div className="flex justify-between py-1">
                      <span>Title Fee</span>
                      <span>{formatCurrency(calculation.title_fee)}</span>
                    </div>
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between py-1">
                      <span>IMF Fee (5%)</span>
                      <span>{formatCurrency(calculation.imf_fee)}</span>
                    </div>
                    <div className="flex justify-between py-1">
                      <span>Sales Assist Fee {saleType === 'Retail' ? '(5%)' : '(0%)'}</span>
                      <span>{formatCurrency(calculation.sales_assist_fee)}</span>
                    </div>
                    <div className="flex justify-between py-1">
                      <span>Brokerage Fee {['Consigned', 'Partner-Owned'].includes(ownershipType) ? '(10%)' : '(0%)'}</span>
                      <span>{formatCurrency(calculation.brokerage_fee)}</span>
                    </div>
                    <div className="flex justify-between py-1">
                      <span>Pack Fee</span>
                      <span>{formatCurrency(calculation.pack_fee)}</span>
                    </div>
                    <div className="flex justify-between py-1 border-t border-gray-200 mt-1 pt-1">
              ...
              <div className="mt-4 pt-3 border-t-2 border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <div className="flex justify-between py-1">
                      <span className="font-medium">Net After Deductions</span>
                      <span className="font-medium text-blue-600">{formatCurrency(calculation.net_after_deductions)}</span>
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between py-1">
                      <span className="font-medium">Net Split {ownershipType === 'Shared' ? '(Net/2)' : '(Brokerage/2)'}</span>
                      <span className="font-medium text-green-600">{formatCurrency(calculation.net_split)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-gray-200 flex justify-end space-x-2">
                {userRole !== 'Admin' && priceStatus === 'Current' && (
                  <button 
                ...
                {userRole === 'Admin' && priceStatus === 'Proposed' && (
                  <>
                    <button 
                      onClick={finalizePrice}
                      className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700"
...
export default ErrorBoundary ? (
  props => (
    <ErrorBoundary>
      <RoleBasedVehicleCalculator {...props} />
    </ErrorBoundary>
  )
) : RoleBasedVehicleCalculator;
